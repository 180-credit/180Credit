...
-- 10/06/2019
CREATE PROCEDURE `viewUserAreasOfSpeciality`() NOT DETERMINISTIC NO SQL SQL SECURITY DEFINER select `u`.`user_id` AS `id`,concat(`ud`.`firstName`,' ',`ud`.`lastName`) AS `name`,`a`.`name` AS `area_name`,`u`.`area_of_specialty_id` AS `area_of_specialty_id` from ((`180_credit`.`areas_of_specialty` `a` left join `180_credit`.`user_areas_of_specialty` `u` on(`u`.`area_of_specialty_id` = `a`.`id`)) left join `180_credit`.`users` `ud` on(`u`.`user_id` = `ud`.`userId`)) where `u`.`user_id` is not null

CREATE PROCEDURE `viewZipCodes`(IN `pQuery` VARCHAR(255)) NOT DETERMINISTIC NO SQL SQL SECURITY DEFINER BEGIN SELECT * FROM (SELECT `zipcodes`.`id`,concat(`zipcodes`.`ZIPCode`,' ',`zipcodes`.`CityName`,', ',`zipcodes`.`StateAbbr`) as name FROM `zipcodes`) as z WHERE `z`.`name` LIKE CONCAT('%',LOWER(TRIM(`pQuery`)),'%'); END